...
public class TokenProvider {
    ...
    //프로세스 흐름 :  yml < JwtProperties.class < TokenProvider
    // ======================================================

    // 0. 호출 후 토큰을 리턴 받는 메소드
    public String generateToken(User user, Duration duration) {
        Date now = new Date();
        return makeToken(new Date(now.getTime() + duration.toMillis()), user);
        // new Date -> expiry, user -> user
        // new Date = now.getTime + Duration.toMillis()
    }//======================================================
    ...
    private String makeToken(Date expiry, User user) {
        ...
        return Jwts.builder()
                ...
                .setSubject(user.getPhoneNumber()) // 유저 이메일 내용에 넣음 -> 전화번호가 키 (아마 여긴 이메일)
    ...
    //     * JWT 토큰을 받아 인증 객체인 Authentication을 생성하는 메서드(구글이나, 세션 로그인도 관련 메서드 존재 했었던 걸로 기억)
